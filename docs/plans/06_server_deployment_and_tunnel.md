...
services:
  co-force:
    ...
    # không publish port — chỉ cloudflared với tới qua network nội bộ
...
- Tunnel dùng **token mode** (tạo tunnel trên Cloudflare dashboard → copy token vào `.env`) — không cần `cloudflared login` interactive trong container.
...
6. **systemd units** (kèm hardening):
   ...
   ```
7. **Backup:** `co-force-backup.timer` daily 03:00 — `sqlite3 ... ".backup"` từng workspace + `config` → tar.zst, giữ 14 bản, verify integrity (`PRAGMA integrity_check` trên bản backup). Tùy chọn bật **Litestream** replicate liên tục lên Cloudflare R2/S3 (hỏi trong installer).
...
1. **Provider CLIs trên server:** cài các CLI được chọn (vd `claude` CLI) chạy dưới user `coforce`; API key vào `secrets.toml` (0600). Smoke test: chạy 1 lệnh headless `claude -p "ping"` xác nhận auth OK.
...
### 4.1 Token model (bảng `api_tokens`)
...
providers = ["claude-code"]                 # CLI đã cài + smoke test lúc install
...
- **Component registry:** `db`, `llm.embedding`, `llm.classifier`, `llm.reasoner`, `tunnel`(kiểm tra cloudflared unit qua D-Bus/systemctl), `disk` (< 10% free → warn). Mỗi component: probe định kỳ 30s + probe khi lỗi runtime.
...
1. Auth: bảng `api_tokens` + repo + `AuthLayer` (unit test: valid/expired/revoked/scope sai) — **làm trước vì WS-B cần**.
