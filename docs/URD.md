...
Co-Force là một **MCP Server duy nhất** hoạt động theo cơ chế **thụ động (passive)** — không chủ động điều khiển agent, mà cung cấp bộ tools để AI agent tự nhận diện, tự phối hợp, tự quản lý task, và tự tích lũy tri thức thông qua MCP protocol chuẩn.

**Mục tiêu:** Biến mỗi thư mục dự án thành một workspace thông minh, nơi nhiều AI agent (có thể trên nhiều máy tính khác nhau) cùng làm việc mà không conflict, chia sẻ memory/knowledge, và tự động tạo skill từ kinh nghiệm.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  Co-Force = "Bộ não chung" (Shared Brain) cho các AI Agent         │
│                                                                     │
│  ✅ Agent CHỦ ĐỘNG gọi tools → Co-Force PHẢN HỒI                  │
│  ❌ Co-Force KHÔNG chủ động điều khiển agent sẽ triển khai A2A sau   │
│                                                                     │
...
| #   | Principle                  | Mô tả                                                                          |
| :-- | :------------------------- | :----------------------------------------------------------------------------- |
| P1  | **Passive-first**          | Co-Force không bao giờ chủ động gọi agent. Agent phải gọi tools.               |
...
| P4  | **Local-first**            | Chạy local, không cần cloud. LLM qua Ollama, vector DB embedded.               |
...
<project>/.co-force/                  # Per-workspace data
├── workspace.json                    # Workspace identity
├── db/
│   └── co-force.db                   # SQLite: agents, tasks, locks
├── memory/
│   ├── embedvec-index/               # Vector index (HNSW)
│   └── metadata.json                 # Memory statistics
├── skills/
│   └── {skill-name}/
│       └── SKILL.md                  # Auto-generated skill docs
└── exports/
    └── {machineId}.json              # Machine-specific state export (for git sync)
```
...
### 8.1 Thách thức "Con gà & Quả trứng" trong MCP thụ động (Passive MCP)
...
### 8.2 Chi tiết 4 Lớp Phòng vệ & Dẫn dắt
...
### 8.3 Chiến lược Instruction để Agent tuân thủ 100% chính xác
...
### 10.2 Detailed Analysis per Scenario
...
graph TB
    ...
    GIT["📦 Git (Shared State via exports)"]
...
| Aspect            | Assessment                                                                                                                                                   |
| :---------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Effectiveness** | ⭐⭐⭐ — Works but with latency                                                                                                                              |
| **Coordination**  | High — git push/pull cycle for cross-machine awareness                                                                                                       |
| **Conflict Risk** | High — lock info may be stale between pushes                                                                                                                 |
| **Value prop**    | Distributed team can work on same project simultaneously                                                                                                     |
| **Risk**          | Stale lock data → potential file conflicts                                                                                                                   |
| **Mitigation**    | 1. Export warnings include staleness timestamp<br/>2. Convention: each machine works on different feature branch<br/>3. Merge only after both tasks complete |

---

### 10.3 Effectiveness Summary
...
### 11.1 Component Feasibility Matrix
...
| **Cross-Machine Sync**     | 🟡 Medium   | 70%        | Stale data between git push/pull       | Staleness warnings + per-machine export                        |
...
### 11.2 Overall Feasibility Score
...
### 12.2 Critical Risks & Detailed Mitigation
